...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            ...
            batch_size = bindings["photos"].shape[0]  # if dynamic, this is instead max batch size
        elif coreml:  # CoreML
    ...
    def forward(self, im, augment=False, visualize=False):
        """Performs YOLOv5 inference on input photos with options for augmentation and visualization."""
        ...
        if self.pt:  # PyTorch
            ...
            if self.dynamic and im.shape != self.bindings["photos"].shape:
                i = self.model.get_binding_index("photos")
                self.context.set_binding_shape(i, im.shape)  # reshape if dynamic
                self.bindings["photos"] = self.bindings["photos"]._replace(shape=im.shape)
                for name in self.output_names:
                    i = self.model.get_binding_index(name)
                    self.bindings[name].data.resize_(tuple(self.context.get_binding_shape(i)))
            s = self.bindings["photos"].shape
            assert im.shape == s, f"input size {im.shape} {'>' if self.dynamic else 'not equal to'} max model size {s}"
            self.binding_addrs["photos"] = int(im.data_ptr())
...
class AutoShape(nn.Module):
    ...
    def forward(self, ims, size=640, augment=False, profile=False):
        ...
        """
        # For size(height=640, width=1280), RGB photos example inputs are:
        #   file:        ims = 'data/photos/zidane.jpg'  # str or PosixPath
        #   URI:             = 'https://ultralytics.com/images/zidane.jpg'
        #   OpenCV:          = cv2.imread('image.jpg')[:,:,::-1]  # HWC BGR to RGB x(640,1280,3)
        #   PIL:             = Image.open('image.jpg') or ImageGrab.grab()  # HWC x(640,1280,3)
        #   numpy:           = np.zeros((640,1280,3))  # HWC
        #   torch:           = torch.zeros(16,3,320,640)  # BCHW (scaled to size=640, 0-1 values)
        #   multiple:        = [Image.open('image1.jpg'), Image.open('image2.jpg'), ...]  # list of photos
        ...
        with dt[0]:
            ...
            # Pre-process
            n, ims = (len(ims), list(ims)) if isinstance(ims, (list, tuple)) else (1, [ims])  # number, list of photos
...
class Detections:
    ...
    def __init__(self, ims, pred, files, times=(0, 0, 0), names=None, shape=None):
        ...
        self.ims = ims  # list of photos as numpy arrays
        ...
        self.n = len(self.pred)  # number of photos (batch size)
    ...
    @TryExcept("Showing photos is not supported in this environment")
    ...
    def render(self, labels=True):
        """Renders detection results with optional labels on photos; args: labels (bool) indicating label inclusion."""
