...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        # Keep tracked of amount of logged photos to enforce a limit
        self.current_epoch_logged_images = set()
        # Maximum number of photos to log to clearML per epoch
        self.max_imgs_to_log_per_epoch = 16
        # Get the interval of epochs when bounding box photos should be logged
    ...
    def log_debug_samples(self, files, title="Debug Samples"):
        """
        Log files (photos) as debug samples in the ClearML task.

        arguments:
        files (List(PosixPath)) a list of file paths in PosixPath format
        title (str) A title that groups together photos with the same values
        """
        for f in files:
            if f.exists():
